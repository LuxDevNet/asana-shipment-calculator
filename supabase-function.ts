...
const getBaseUrl = (reqUrl: string) => {
  ...
  // Supabase functions have the format: /functions/v1/function-name
  // We keep the prefix path so relative routing is clean
  return `${url.protocol}//${url.host}${url.pathname.split("/").slice(0, 4).join("/")}`;
};

// 3. Supabase Client Setup (For database access)
...
app.get("/widget", async (c) => {
  ...
  // Extract shipment ID from the URL (e.g. /shipments/SH-1001)
